...
help:
	...
	@echo "Run \033[1m'make dns_edit'\e[0m to add my-nginx.internal to /etc/hosts (it is included in make deploy)"
	@echo "Run \033[1m'make destroy'\e[0m to delete project"
	@@echo "Run \033[1m'make dns_revert'\e[0m to revert /etc/hosts state (it is included in make destroy)"

init:
	@cd terraform/vpc; terraform init && cd $$OLDPWD
	...
	@cd terraform/cert; terraform init && cd $$OLDPWD
	@cd terraform/alb; terraform init && cd $$OLDPWD
	@cd terraform/asg; terraform init && cd $$OLDPWD

apply:
	@cd terraform/vpc; terraform apply -auto-approve  && cd $$OLDPWD
	...
	@cd terraform/cert; terraform apply -auto-approve && cd $$OLDPWD
...
remove:
	...
	@cd terraform/cert; terraform destroy -auto-approve && cd $$OLDPWD
	...
	@cd terraform/vpc; terraform destroy -auto-approve && cd $$OLDPWD
